...
public class GBTCBenchmarkTest {
    ...
    @Test
    public void testAgaricus() throws Exception {
        Case c = new Case();
        c.dataDir = dataDir;
        c.trainFile = "agaricus.txt.train.txt";
        c.testFile = "agaricus.txt.test.txt";
        c.colNames = new String[] {"label", "vec"};
        c.colTypeStrs = new String[] {"DOUBLE", "STRING"};
        c.needDeserializeVector = true;
        c.vectorSize = 128;

        c.labelCol = "label";
        c.featureCols = new String[] {"vec"};
        c.maxDepth = 6;
        c.maxIter = 50;
    ...
    public void testAvazu() throws Exception {
        ...
        c.labelCol = "click";
        c.featureCols =
                ArrayUtils.removeElements(c.colNames, "id", "click", "device_id", "device_ip");
